...
const navItems = [
  { label: "Đơn hàng", icon: OrderIcon, href: "/account/update-profile" },
  { label: "Danh sách sản phẩm", icon: ProductListIcon, href: "/account/list-product" },
  { label: "Thêm sản phẩm", icon: AddProductIcon, href: "/account/add-product" },
  { label: "Thêm mã giảm giá", icon: AddProductIcon, href: "/account/add-coupon" },
  { label: "Ví tiền", icon: Wallet, href: "/account/wallet" },
  { label: "Chỉnh sửa sản phẩm", icon: EditProductIcon, href: "/account/update-product" },
  { label: "Báo cáo doanh số", icon: ReportIcon, href: "/account/sales-report" },
...
const Sidebar = ({ isSidebarOpen, setIsSidebarOpen }) => {
  ...
  return (
    <aside
      className={`fixed top-16 left-0 bottom-0 bg-white shadow-lg flex flex-col z-40 overflow-hidden transition-all duration-300 ease-in-out ${isSidebarOpen ? "w-64" : "w-16"
        }`}
    >
      {/* Nút thu gọn/mở rộng */}
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.label}
                to={item.href}
                className={`group flex items-center rounded-lg px-3 py-2 relative transition-all duration-200 ${isActive
              ...
              >
                <Icon
                  className={`flex-shrink-0 transition-all duration-300 ${isSidebarOpen ? "mr-3" : "mx-auto"
                    }`}
                  style={{ fontSize: 20 }}
                />
                {isSidebarOpen && (
