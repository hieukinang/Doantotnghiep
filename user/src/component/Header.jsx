...
const Header = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`http://localhost:8080/api/products?search=${searchQuery}`);
            ...
            {searchQuery && (
              <div className="absolute left-0 top-full w-full bg-white border rounded-lg shadow-md mt-1 max-h-80 overflow-y-auto z-50">
                {loading ? (
                  ...
                  <p className="p-3 text-gray-500 text-sm">Không tìm thấy sản phẩm</p>
                ) : (
                  products.map((p) => (
                    <Link
                      key={p.id}
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-[#116AD1] font-semibold text-sm">₫{p.price.toLocaleString()}</p>
                      </div>
                    </Link>
                  ))
                )}
