...
const ShipperManagement = () => {
  ...
  useEffect(() => {
    const fetchStores = async () => {
      try {
        ...
        const res = await axios.get(`${backendURL}/shippers/processing`, {
          headers: {
        ...
        toast.error("Không thể tải danh shipper");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleApprove = async (id) => {
    try {
      ...
      const res = await axios.patch(
        `${backendURL}/shippers/update-status/${id}`,
        { status: "ACTIVE" },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      if (res.data?.status === "success") {
        toast.success("Duyệt shipper thành công!");
        setStores((prev) =>
          prev.map((s) => (s.id === id ? { ...s, status: "ACTIVE" } : s))
        );
      }
    } catch (err) {
      toast.error("Không thể duyệt shipper");
    }
  };
  ...
  const handleDisapprove = async (id) => {
    try {
      ...
      if (res.data?.status === "success") {
        toast.success("Bỏ duyệt shipper thành công!");
        setStores((prev) =>
          prev.map((s) => (s.id === id ? { ...s, status: "PROCESSING" } : s))
        );
      }
    } catch (err) {
      toast.error("Không thể bỏ duyệt shipper");
    }
  };
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-blue-600 text-white">
            <tr>
              ...
              <th className="p-3 text-left w-[250px]">Tên Shipper</th>
              <th className="p-3 text-left w-[300px]">Số điện thoại</th>
              ...
              currentStores.map((store, index) => {
                ...
                return (
                  <tr key={store.id} className="border-t hover:bg-gray-50">
                    ...
                    <td className="p-3 text-left">{store.fullname}</td>
                      ...
                      <select
                        ...
                        onChange={async (e) => {
                          ...
                          try {
                            const token = localStorage.getItem("adminToken");
                            const res = await axios.patch(
                              `${backendURL}/shippers/update-status/${store.id}`,
                              { status: newStatus },
                              { headers: { Authorization: `Bearer ${token}` } }
                            );
